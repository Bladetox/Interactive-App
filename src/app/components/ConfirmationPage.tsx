...
interface CartItem {
  ...
  price: string;
  priceValue: number;
  image: string; // now holds an emoji string
  quantity: number;
}
    ...
    <div className="flex flex-col min-h-screen bg-white w-full max-w-[412px] mx-auto">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          ...
          <h1 className="text-lg font-semibold text-green-700">Shopping List</h1>
          <div className="relative p-2">
            <ShoppingBasket className="w-6 h-6 text-gray-700" />
            {cartCount > 0 && (
              <span className="absolute -top-1 -right-1 bg-green-600 text-white text-[9px] rounded-full w-4 h-4 flex items-center justify-center">
                {cartCount}
              </span>
            )}
          </div>
        ...
        <div className="flex items-center justify-between px-4 pb-3">
          <h2 className="text-2xl font-semibold tracking-tight text-gray-900">Confirmation</h2>
          <button onClick={onBack} className="flex items-center gap-1 text-green-700 text-sm">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4 pb-56 divide-y divide-gray-100">
        {cartItems.map(item => (
          <div key={item.id} className="flex items-center gap-4 py-4">
            <div className="w-16 h-16 flex items-center justify-center bg-gray-50 rounded-xl text-4xl shrink-0">
              {item.image}
            </div>
            ...
            <p className="text-sm text-gray-500 w-16 text-right">
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[412px] bg-white border-t border-gray-100 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] px-6 pt-4 pb-6">
        ...
        <button
          ...
          className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold hover:bg-green-700 transition-colors flex items-center justify-between px-5"
        >
